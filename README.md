...
Phase 1 — core site + backend built. Auth (register / login / logout / password reset / email verification), the profile lifecycle, user roles (`user` / `admin`), full CRUD for the Postgraduate Opportunities board, and the Contact form are all wired to a real versioned API (`/api/v1`) with a migration system and 51 passing tests. The `scholarships.html` board now renders live from `GET /api/v1/opportunities`, and the contact form persists submissions and notifies the team (console-logged in dev — see `backend/src/lib/mailer.js`). See [PROJECT-DOCS.md §13](PROJECT-DOCS.md).
